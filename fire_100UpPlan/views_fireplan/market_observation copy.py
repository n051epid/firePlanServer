...
from django.db.models import IntegerField, Min, Max, F, Q, Value
...
class ConvertibleBondMarketDataView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # 可转债市场数据
            cb_market_data = self._get_convertible_bond_market_data()
            # 强赎可转债列表
            redemption_bonds = self._get_redemption_convertible_bonds()

            convertible_bond_market_data = {
                "cb_index_data": cb_index_data,
                "cb_market_data": cb_market_data,
                "redemption_bonds": redemption_bonds
            }
    ...
    def _get_convertible_bond_market_data(self):
        """获取可转债市场数据"""
        try:
            # 获取最新日期
            ...
            convertible_bond_data = BondData.objects.filter(
                ...
                models.Q(maturity_date__lte=datetime.now().date() + timedelta(days=90))  # 到期时间早于未来三个月
            ).values(
                'code',
                'name',
                'close',
        ...
        try:
            callable_bonds = BondData.objects.annotate(
                first_two_numbers=Cast(
                    Substr('redemption_countdown', 1, 2), # 创造一个字段，取 redemption_countdown 的前两位
                    output_field=IntegerField()
                )
            ).filter(
                models.Q(is_callable__in=['已公告强赎', '公告要强赎', '已满足强赎条件']) |
                (
                    models.Q(redemption_countdown__regex=r'^\d{2}') &
                    models.Q(first_two_numbers__gte=10)
                )
            ).exclude(
                models.Q(last_trading_date__lt=datetime.now().date())  # 最后交易日早于今天
            ).annotate(
                display_countdown=models.Case(
                    models.When(is_callable='公告要强赎', then=models.Value('公告要强赎')),
                    default=models.F('redemption_countdown'),
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            response = StreamingHttpResponse(
                self.chat_with_kimi([{"role": "user", "content": message}]),
